...
#include "../OnMei/OnMei/Pitch/a_Body.hpp"
...
inline bool Haichi::IsNaurallyOrdered() noexcept { return CheckOrderedWithinRange( m_bas.GetNoteNumber() , m_ten.GetNoteNumber() , m_alt.GetNoteNumber() , m_sop.GetNoteNumber() ) && HasNaturalOnDo(); }

inline bool Haichi::CheckHasDoubleDaiSanOn( const uint& bas_num , const uint& ten_num , const uint& alt_num , const uint& sop_num ) const noexcept { return ( bas_num * ten_num * alt_num * sop_num ) % 9 == 0; }

inline bool Haichi::CheckHasDaiGoOn( const uint& bas_num , const uint& ten_num , const uint& alt_num , const uint& sop_num ) const noexcept { return ( bas_num * ten_num * alt_num * sop_num ) % 5 == 0; }
...
inline bool Haichi::HasNaturalOnDo() const noexcept { return OnDo( m_bas , m_ten ).GetDoSuu() <= 12 && OnDo( m_ten , m_alt ).GetDoSuu() <= 8 && OnDo( m_al , m_sop ).GetDoSuu() <= 8; }
