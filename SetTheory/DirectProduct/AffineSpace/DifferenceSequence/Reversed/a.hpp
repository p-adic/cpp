...
// https://yukicoder.me/submissions/972228（Abstract、零初期化、区間加算、一点取得）

// 数列に対するimos法
// https://imoz.jp/algorithms/imos_method.html
// を森上の可換群に値を持つ関数に一般化したデータ構造を反転したもの。
// 葉からの（高階）階差数列を管理する。
...
// 一点代入O(min(|m_degree|,|m_degree-1|)*size)
// 一点加算O(min(|m_degree|,|m_degree-1|)|m_degree|*size)
// 始切片加算O(|m_degree-1|*size)
// 区間加算O(|m_degree-1|*size)

// 一点取得O(|m_degree|*size)/O(m_G.Edge(t).size() + |m_degree+1|*size)
// 終切片和取得O(|m_degree+1|*size)
// 部分木和取得O(t_outside.size() + |m_degree+1|*size)
...
// 一点取得O(min(|m_degree|,|m_degree-1|)*size)
