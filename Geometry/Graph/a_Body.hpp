...
template <typename T , typename E> inline MemorisationGraph<T,E>::MemorisationGraph( const int& size , const T& dummy , E edge ) : EdgeImplimentation<T,T,const int&,E>( size , move( edge ) ) , m_length() , m_memory() , m_memory_inv() { static_assert( is_invocable_v<E,T> ); }
