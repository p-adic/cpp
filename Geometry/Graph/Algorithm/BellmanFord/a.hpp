// c:/Users/user/Documents/Programming/Mathematics/Geometry/Graph/Algorithm/BellmanFord/a.hpp
...
public:
  ...
  //  格納した配列を第2成分に返す。
  tuple<vector<bool>,vector<U>> GetDistance( const T& t_start , const bool& many_edges = false , int path_length = -1 );
  ...
  template <template <typename...> typename V> tuple<vector<bool>,vector<U>,vector<list<T>>> GetPath( const T& t_start , const V<T>& t_finals , const bool& many_edges = false , int path_length = -1 );
  // 各点に対し
  //   「到達可能ならば負の閉路を経由できない」の真偽を格納した配列を第1成分に、
  // 各点に対し
  //   「到達可能?負の閉路を経由する?辺の本数path_length以下での最短経路長:最短経路長:infty」を
  ...
  // 各点に対し
  //   「到達可能かつ負の閉路を経由できない?最短経路:空列」を格納した配列を第3成分に返す。
  tuple<vector<bool>,vector<U>,vector<list<T>>> GetPath( const T& t_start , const bool& many_edges = false , int path_length = -1 );
