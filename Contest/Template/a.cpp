...
IN VE<T2<int>> EdgeOnGrid(CO T2<int>& v){VE<T2<int>>AN{};auto[i,j]=v;if(i>0&&grid[i-1][j]==walkable){AN.push_back({i-1,j});}if(i+1<H&&grid[i+1][j]==walkable){AN.push_back({i+1,j});}if(j>0&&grid[i][j-1]==walkable){AN.push_back({i,j-1});}if(j+1<W&&grid[i][j+1]==walkable){AN.push_back({i,j+1});}RE AN;}
IN VE<pair<T2<int>,ll>> WEdgeOnGrid(CO T2<int>& v){VE<pair<T2<int>,ll>>AN{};auto[i,j]=v;if(i>0&&grid[i-1][j]==walkable){AN.push_back({{i-1,j},1});}if(i+1<H&&grid[i+1][j]==walkable){AN.push_back({{i+1,j},1});}if(j>0&&grid[i][j-1]==walkable){AN.push_back({{i,j-1},1});}if(j+1<W&&grid[i][j+1]==walkable){AN.push_back({{i,j+1},1});}RE AN;}
