// 本来は../../Template/a.cppを適切に利用することで提出コードにライブラリ探索機能を埋め込むが、
// 一応単体でも機能する。以下は単体で使用する場合の例。

#include <bits/stdc++.h>
using namespace std;

// 本来はコンパイル時に-DDEBUG=1を渡すことで提出時には消えるようにする。
#define DEBUG
...
// 2024/11/25時点での
// #include "Contest/AutoCheck/a_Macro.hpp"
#define ASK_NUMBER( ... )						\
  while( true ){                                                        \
    CERR( "" );								\
    CERR( "何番を選択しますか？" );					\
...
// 2024/11/25時点での
// #include "Contest/AutoCheck/LibrarySearch/a.hpp"
AC( LibrarySearch );
...
// 2024/11/25時点での
// #include "Contest/AutoCheck/LibrarySearch/a.cpp"
AC( LibrarySearch )
{
  ASK_NUMBER(
	     "明示式の計算問題" ,
...
AC( ConstructionArray )
{
  ...
  if( num == num_temp++ ){
    ...
    CERR( "- 順列の構成は、階乗進法や互換表示や巡回置換表示を検討し、" );
    CERR( "  小さいケースに帰着させるためにnext_permutationで実験しましょう。" );
  } else {
    CALL_AC( ConstructionPartition );
  }
}
...
// 本来は../../Template/a.cppのように提出コードにライブラリ探索機能を埋め込む。
int main()
{
  int num = 0;
  int num_temp = 0;
  ...
  bool sample_check = false;
  string problem_order = "dummy"; 
  LibrarySearchAutoCheck( num , num_temp , reply , problems , problems_size , use_getline , sample_check , problem_order );
}
