...
template <typename INT , typename RET_NOR , typename RET_DEN , template <typename...> typename DATA_STR> inline int VirtualBoundedLineSubset<INT,RET_NOR,RET_DEN,DATA_STR>::InitialSegmentCount( const INT& i_final ) { assert( InRange( i_final ) ); return m_ds.InitialSegmentSum( Normalise( i_final ) ); }
template <typename INT , typename RET_NOR , typename RET_DEN , template <typename...> typename DATA_STR> inline int VirtualBoundedLineSubset<INT,RET_NOR,RET_DEN,DATA_STR>::IntervalCount( const INT& i_start , const INT& i_final ) { assert( InRange( i_start ) && InRange( i_final ) ); return m_ds.IntervalSum( Normalise( i_start ) , Normalise( i_final ) ); }
...
template <typename INT , typename RET_NOR , typename RET_DEN , template <typename...> typename DATA_STR> inline typename VirtualBoundedLineSubset<INT,RET_NOR,RET_DEN,DATA_STR>::iterator VirtualBoundedLineSubset<INT,RET_NOR,RET_DEN,DATA_STR>::MaximumLeq( const INT& i , const int& k ) { const INT num = InitialSegmentCount( i ) - k; const INT l = Denormalise( m_ds.Search( [&]( const INT& sum , const int& j ){ return num <= sum; } ) ); return num >= 0 && find( l ) ? typename VirtualBoundedLineSubset<INT,RET_NOR,RET_DEN,DATA_STR>::iterator{ *this , l } : end(); }
...
template <typename INT , typename RET_NOR , typename RET_DEN , template <typename...> typename DATA_STR> inline typename VirtualBoundedLineSubset<INT,RET_NOR,RET_DEN,DATA_STR>::iterator VirtualBoundedLineSubset<INT,RET_NOR,RET_DEN,DATA_STR>::MinimumGt( const INT& i , const int& k ) { const INT num = InitialSegmentCount( i ) + k; const INT r = Denormalise( m_ds.Search( [&]( const INT& sum , const int& j ){ return num < sum; } ) ); return find( r ) ? typename VirtualBoundedLineSubset<INT,RET_NOR,RET_DEN,DATA_STR>::iterator{ *this , r } : end(); }
