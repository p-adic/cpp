...
template <typename INT , template <typename...> typename DATA_STR> inline AbstractNonNegativeLineSubset<INT,DATA_STR>::AbstractNonNegativeLineSubset( const INT& ubound ) { assert( -1 <= ubound ); this->m_lbound = 0; this->m_ubound = ubound; this->m_ds.Initialise( this->m_ubound + 1 ); }
