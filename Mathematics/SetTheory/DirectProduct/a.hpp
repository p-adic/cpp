...
public:
  ...
  virtual ~DirectProduct() = default;
  inline const T0& GetInitial() const noexcept;
  inline const DirectProduct<>& IgnoreInitial() const noexcept;
...
public:
  ...
  virtual ~DirectProduct() = default;
  inline const T0& GetInitial() const noexcept;
  inline const DirectProduct<T1,T2...>& IgnoreInitial() const noexcept;
...
template <typename T0 , typename... T1> inline const T0& SeparateInitialLeft( const DirectProduct<T0,T1...>& ) noexcept;
template <typename T0 , typename... T1> inline const DirectProduct<T1...>& SeparateInitialRight( const DirectProduct<T0,T1...>& ) noexcept;
template <typename T0> inline const TypeOfIgnoreFinal<T0>& SeparateFinalLeft( const DirectProduct<T0>& ) noexcept;
template <typename T0 , typename T1 , typename... T2> inline TypeOfIgnoreFinal<T0,T1,T2...> SeparateFinalLeft( const DirectProduct<T0,T1,T2...>& );
template <typename T0> inline const TypeOfFinal<T0>& SeparateFinalRight( const DirectProduct<T0>& ) noexcept;
...
template <uint i , typename T0 , typename T1 , typename T2 , typename... T3> auto RestrictToLeft( const DirectProduct<T0,T1,T2,T3...>& v ) -> typename enable_if< less<uint>()( 1 ,  i ) && less<uint>()( i , 3 + sizeof...( T3 ) ) , TypeOfRestrictToLeft<i,T0,T1,T2,T3...> >::type;
