...
template <typename T , int N> inline T IntervalAddBIT<T,N>::IntervalAdd( const int& i_start , const int& i_final , const T& n ) { for( int i = i_start ; i <= i_final ; i++ ){ m_a[i] += n; } m_bit_0.Add( i_start , - n * ( i_start - 1 ) ); m_bit_0.Add( i_final + 1 , n * i_final ); m_bit_1.Add( i_start , n ); m_bit_0.Add( i_final + 1 , - n ); }
