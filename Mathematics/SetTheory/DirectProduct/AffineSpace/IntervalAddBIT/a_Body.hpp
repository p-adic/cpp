...
template <typename T , int N> inline IntervalAddBIT<T,N>::IntervalAddBIT() : m_a() , m_bit_0() , m_bit_1() {}
template <typename T , int N> inline IntervalAddBIT<T,N>::IntervalAddBIT( const T ( & a )[N] ) : m_a() , m_bit_0() , m_bit_1() { operator+=( a ); }

template <typename T , int N> inline const T& IntervalAddBIT<T,N>::operator[]( const int& i ) const { return m_a[i]; }
template <typename T , int N> inline void IntervalAddBIT<T,N>::Set( const int& i , const T& n ) { Add( i , n - m_a[i] ); }
...
template <typename T , int N> inline void IntervalAddBIT<T,N>::IntervalAdd( const int& i_start , const int& i_final , const T& n ) { for( int i = i_start ; i <= i_final ; i++ ){ m_a[i] += n; } m_bit_0.Add( i_start , - n * ( i_start - 1 ) ); m_bit_0.Add( i_final + 1 , n * i_final ); m_bit_1.Add( i_start , n ); m_bit_0.Add( i_final + 1 , - n ); }
