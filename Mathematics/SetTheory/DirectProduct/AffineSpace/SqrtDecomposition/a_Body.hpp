...
template <typename T , int N , int N_sqrt> inline constexpr SqrtDecomposition<T,N,N_sqrt>::SqrtDecomposition( const T ( &a )[N] ) : m_a() , m_b() { int i = 0; for( int d = 0 ; d < N_d ; d++ ){ int& m_bd = m_b[d]; for( int j = 0 ; j < N_sqrt ; j++ ){ m_bd += m_a[i] = a[i]; i++; } } while( i < N ){ m_a[i] = a[i]; i++; } }

template <typename T , int N , int N_sqrt> inline constexpr void SqrtDecomposition<T,N,N_sqrt>::Set( const int& i , const T& n ) { int& m_ai = m_a[i]; if( i < N_m ){ ( m_b[i / N_d] -= m_ai ) += n; } m_ai = n; }
