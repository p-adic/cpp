...
template <typename R , typename PT_COMM_MAGMA , typename U , typename R_MODULE> template <typename...Args> inline CommutativeDualSqrtDecomposition<R,PT_COMM_MAGMA,U,R_MODULE>::CommutativeDualSqrtDecomposition( PT_COMM_MAGMA L , R_MODULE X , Args&&... args ) : DualSqrtDecomposition<R,PT_COMM_MAGMA,U,R_MODULE>( move( L ) , move( X ) , forward<decay_t<Args>>( args )... ) {}
