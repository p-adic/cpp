...
template <typename R , typename PT_MAGMA , typename U , typename R_MODULE> template <typename...Args> inline void DualSqrtDecomposition<R,PT_MAGMA,U,R_MODULE>::Reset( Args&&... args ) { *this = DualSqrtDecomposition<R,PT_MAGMA,U,R_MODULE>( move( m_L ) , move( m_X ) , forward<Args>( args )... ); }
