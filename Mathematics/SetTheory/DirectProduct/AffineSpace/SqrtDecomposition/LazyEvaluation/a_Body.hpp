...
template <typename R , typename PT_MAGMA , typename U , typename R_MODULE> inline void LazySqrtDecomposition<R,PT_MAGMA,U,R_MODULE>::IntervalSet( const int& i_start , const int& i_final , const U& u )
{
  ...
  if( i_min < i_0 ){
    ...
    if( m_suspended_d ){
      ...
      IntervalSet_Body( i_min , i_0 , u );
      m_bd = m_M.Product( m_M.Product( IntervalProd_Body( d_0_N_sqrt_minus , i_min ) , m_M.Power( u , i_0 - i_min ) ) , IntervalProd_Body( i_0 , d_0_N_sqrt ) );
  ...
  if( i_1 < i_ulim ){
    ...
    if( m_suspended_d ){
      ...
      IntervalSet_Body( i_1 , i_ulim , u );
      m_bd = m_M.Product( m_M.Product( IntervalProd_Body( d_1_N_sqrt , i_1 ) , m_M.Power( u , i_ulim - i_1 ) ) , IntervalProd_Body( i_ulim , d_1_N_sqrt_plus ) );
...
template <typename R , typename PT_MAGMA , typename U , typename R_MODULE> inline void LazySqrtDecomposition<R,PT_MAGMA,U,R_MODULE>::IntervalAct( const int& i_start , const int& i_final , const R& r )
{
  ...
  if( r != m_L.Point() ){
	...
	if( m_lazy_action_d == m_L.Point() ){
	  ...
	  IntervalAct_Body( i_min , i_0 , m_R( t , m_lazy_action_d ) );
...
template <typename R , typename PT_MAGMA , typename U , typename R_MODULE> inline U LazySqrtDecomposition<R,PT_MAGMA,U,R_MODULE>::IntervalProd_Body( const int& i_min , const int& i_ulim )
{
...
template <typename R , typename PT_MAGMA , typename U , typename R_MODULE> inline U LazySqrtDecomposition<R,PT_MAGMA,U,R_MODULE>::IntervalProduct( const int& i_start , const int& i_final )
{
  ...
  if( i_min < i_0 ){
    ...
    // この時d_0 > 0になる。
    const int d_0_minus = d_0 - 1;
    answer = m_suspended[d_0_minus] ? m_M.Power( m_lazy_substitution[d_0_minus] , i_0 - i_min ) : m_M.Action( m_lazy_action[d_0_minus] , IntervalProd_Body( i_min , i_0 ) );
  ...
  if( i_1 < i_ulim ){
    ...
    // この時d_1 < m_N_dになる。
    answer = m_M.Product( answer , m_suspended[d_1] ? m_M.Power( m_lazy_substitution[d_1] , i_ulim - i_1 ) : m_M.Action( m_lazy_action[d_1] , IntervalProd_Body( i_1 , i_ulim ) ) );
