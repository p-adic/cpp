...
template <TEMPLATE_ARGUMENTS_FOR_LAZY_SQRT_DECOMPOSITION> inline constexpr T NonCommutativeLazySqrtDecomposition<T,m_T,e_T,U,o_U,N,N_sqrt>::Get( const int& i ) const { return i < N_m ? o_U( m_b[i/N_sqrt] , m_a[i] ) : m_a[i]; }
...
template <TEMPLATE_ARGUMENTS_FOR_LAZY_SQRT_DECOMPOSITION> inline constexpr void NonCommutativeLazySqrtDecomposition<T,m_T,e_T,U,o_U,N,N_sqrt>::IntervalAct( const int& i_start , const int& i_final , const T& n )
{
  ...
  if( n != g_e ){
	...
	for( int i = d_0_N_sqrt - N_sqrt ; i < d_0_N_sqrt_ ; i++ ){
	...
	for( int i = d_1_N_sqrt + N_sqrt - 1 ; i >= d_1_N_sqrt_ ; i-- ){
