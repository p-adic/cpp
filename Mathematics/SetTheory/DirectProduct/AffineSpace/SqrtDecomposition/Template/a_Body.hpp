...
template <TEMPLATE_ARGUMENTS_FOR_ABSTRACT_SQRT_DECOMPOSITION> const T& AbstractSqrtDecomposition<T,m_T,e_T,i_T,N,N_sqrt>::g_e = e_T();
...
template <TEMPLATE_ARGUMENTS_FOR_ABSTRACT_SQRT_DECOMPOSITION> inline constexpr AbstractSqrtDecomposition<T,m_T,e_T,i_T,N,N_sqrt>::AbstractSqrtDecomposition( T ( &&a )[N] ) : m_a() , m_b() { swap( m_a , a ); int i = 0; for( int d = 0 ; d < N_d ; d++ ){ T& m_bd = m_b[d] = g_e; for( int j = 0 ; j < N_sqrt ; j++ ){ m_bd = m_T( m_bd , m_a[i++] ); } } }
