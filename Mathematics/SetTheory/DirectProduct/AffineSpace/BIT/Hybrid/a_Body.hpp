...
template <typename U , typename ABELIAN_GROUP> inline AbstractHybridBIT<U,ABELIAN_GROUP>::AbstractHybridBIT( const AbstractHybridBIT<U,ABELIAN_GROUP>& bit ) : AbstractBIT<U,AdditiveGroup<U>>( bit ) , m_a( bit.m_a ) {}
...
template <typename U , typename ABELIAN_GROUP> inline AbstractHybridBIT<U,ABELIAN_GROUP>& AbstractHybridBIT<U,ABELIAN_GROUP>::operator+=( const vector<U>& a ) { AbstractBIT<U,ABELIAN_GROUP>::operator+=( a ); for( int i = 0 ; i < this->m_size ; i++ ){ U& m_ai = m_a[i]; m_ai = this->m_M.Sum( move( m_ai ) , a[i] ); } return *this; }
