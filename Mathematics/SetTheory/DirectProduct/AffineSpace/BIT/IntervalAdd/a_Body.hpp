...
template <typename U , typename Z_MODULE> AbstractIntervalAddBIT<U,Z_MODULE>::AbstractIntervalAddBIT( Z_MODULE M , const vector<U>& a ) : m_M( move( M ) ) , m_bit_0( m_M ) , m_bit_1( m_M )
{
  ...
  for( int i = 1 ; i < size ; i++ ){
    ...
    U& diff_i = diff[i];
    diff_i = m_M.ScalarProduct( 1 - i , diff_i );
...
template <typename U , typename Z_MODULE> template <typename...Args> inline void AbstractIntervalAddBIT<U,Z_MODULE>::Initialise( const Args&... args ) { *this = AbstractIntervalAddBIT<U,Z_MODULE>( move( m_M ) , args... ); }
