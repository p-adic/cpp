...
template <typename T> inline T IntervalAddBIT<T>::operator[]( const int& i ) const { assert( 0 <= i && i < m_bit_0.size() ); return IntervalSum( i , i ); }
template <typename T> inline T IntervalAddBIT<T>::Get( const int& i ) const { return operator[]( i ); }
