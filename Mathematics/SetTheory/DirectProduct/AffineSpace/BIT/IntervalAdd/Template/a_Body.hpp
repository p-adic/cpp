...
template <TEMPLATE_ARGUMENTS_FOR_BIT> inline T AbstractIntervalAddBIT<T,m_T,e_T,i_T,N>::InitialSegmentSum( const int& i_final ) { return m_T( m_bit_0.InitialSegmentSum( i_final ) , prod( m_bit_1.InitialSegmentSum( i_final ) , i_final ) ); }

template <TEMPLATE_ARGUMENTS_FOR_BIT> inline T AbstractIntervalAddBIT<T,m_T,e_T,i_T,N>::IntervalSum( const int& i_start , const int& i_final ) { return m_T( i_T( InitialSegmentSum( i_start - 1 ) ) , InitialSegmentSum( i_final ) ); }

template <TEMPLATE_ARGUMENTS_FOR_BIT>  inline T AbstractIntervalAddBIT<T,m_T,e_T,i_T,N>::prod( const T& n , int exponent ) { T answer = AbstractBIT<T,m_T,e_T,i_T,N>::g_e; while( exponent-- != 0 ){ answer = m_T( answer , n ); } return answer; }
