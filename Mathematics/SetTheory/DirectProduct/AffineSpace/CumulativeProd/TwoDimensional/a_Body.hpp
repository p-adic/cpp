...
TwoDimensionalCumulativeSum<U,ABELIAN_GROUP>::TwoDimensionalCumulativeSum( const vector<vector<U>>& a , ABELIAN_GROUP M ) :
  m_size_X( a.size() ) , m_size_Y() , m_M( move( M ) ) , m_a( m_size_X + 1 )
{
...
template <typename U , typename ABELIAN_GROUP> inline const U& TwoDimensionalCumulativeSum<U,ABELIAN_GROUP>::InitialRectangleSum( const int& i_x , const int& i_y ) const { assert( -1 <= i_x && i_x < m_size_X && -1 <= i_y && i_y < m_size_Y ); return m_a[i_x+1][i_y+1]; }

template <typename U , typename ABELIAN_GROUP> inline U TwoDimensionalCumulativeSum<U,ABELIAN_GROUP>::RectangleSum( const int& i_start_x , const int& i_start_y , const int& i_final_x , const int& i_final_y )
