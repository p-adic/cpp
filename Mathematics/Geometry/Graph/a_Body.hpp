...
template <typename T , typename E> inline MemorisationGraph<T,E,PTR>::MemorisationGraph( const int& size , E edge ) : EdgeImplimentation<T,T,const int&,E>( size , move( edge ) ) , m_length() , m_memory() , m_memory_inv() { static_assert( is_invocable_v<E> && is_invocable_v<E,T> ); }
...
template <typename T , typename E> inline T MemorisationGraph<T,E,PTR>::Enumeration( const int& i ) { assert( 0 <= i && i < m_length ); return m_memory[i]; }
...
template <typename T , typename E> inline const int& MemorisationGraph<T,E,PTR>::Enumeration_inv( const T& t )
{
...
template <typename T , typename E> inline void MemorisationGraph<T,E,PTR>::Reset() { m_length = 0; m_memory.clear(); m_memory_inv.clear(); }

template <typename T , typename R1 , typename R2 , typename E> inline CRI EdgeImplimentation<T,R1,R2,E>::size() const noexcept { return m_size; }
...
template <typename T , typename E> template <typename F> inline MemorisationGraph<T,F> MemorisationGraph<T,E,PTR>::GetGraph( F edge ) const { return MemorisationGraph( this->size() , move( edge ) ); }
