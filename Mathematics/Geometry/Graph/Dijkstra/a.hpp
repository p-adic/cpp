...
public:
  ...
  U GetDistance( const inner_t<GRAPH>& t_start , const inner_t<GRAPH>& t_final , const bool& many_edges = true , int path_length = -1 );
  vector<U> GetDistance( const inner_t<GRAPH>& t_start , const bool& many_edges = true , int path_length = -1 );
  // 最短経路長の確定している点集合をfixedで指定し、weightに既に格納されている値を重みとする
  // t_startからの有向辺を追加した時の最短経路長を格納し直す。
  void SetDistance( vector<U>& weight , vector<bool>& fixed , const inner_t<GRAPH>& t_start , const bool& many_edges = true , int path_length = -1 );

  // 経路が存在する場合は最短径路長を、存在しない場合はinftyを第1成分に、
  // 経路が存在する場合は最短径路を、存在しない場合は空列を第2成分に返す。
  pair<U,list<inner_t<GRAPH>>> GetPath( const inner_t<GRAPH>& t_start , const inner_t<GRAPH>& t_final , const bool& many_edges = true , int path_length = -1 );
  // t_finalsに属すとは限らない（注意！）各点に対し
  //   「経路が存在する?最短径路長:infty」を格納した配列を第1成分に、
  // t_finalsに属す各点に対し
  //   「経路が存在する?最短径路:空列」を格納した配列を第2成分に返す。
  template <template <typename...> typename V> pair<vector<U>,vector<list<inner_t<GRAPH>>>> GetPath( const inner_t<GRAPH>& t_start , const V<inner_t<GRAPH>>& t_finals , const bool& many_edges = true , int path_length = -1 );
