...
template <typename VALUE , typename INT1 , typename COST , typename INT2> inline vector<ret_t<VALUE,int,int>> NegativeValueUnboundedChoiceMulticaseUnstableKnapsack( const int& N , VALUE value , COST cost , const INT1& cost_sum_bound , const vector<INT2>& choice_stability_num );
// 計算量O(N * choice_stability_num * cost_sum_bound)で
// 各iに対し項目iを無制限に重複選択でき1+m回目の重複選択における項目iの
// 価値がvalue(i,min(m,choice_stable_num[i]))であり
// コストがcost(i,min(m,choice_stable_num[i]))であるとして、
...
// それを実現するコストの最小値を求める。
template <typename U , typename COMM_MONOID , typename VALUE , typename INT1 , typename COST , typename INT2> inline pair<U,INT1> AbstractNegativeValueUnboundedChoiceUnstableKnapsack( COMM_MONOID M , const int& N , VALUE value , const U& exceptional , COST cost , const INT1& cost_sum_bound , const vector<INT2>& choice_stability_num );
template <typename VALUE , typename INT1 , typename COST , typename INT2> inline pair<ret_t<VALUE,int,int>,INT1> NegativeValueUnboundedChoiceUnstableKnapsack( const int& N , VALUE value , COST cost , const INT1& cost_sum_bound , const vector<INT2>& choice_stability_num );
