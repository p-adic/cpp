...
// を満たす場合にのみサポート。
// 計算量O(2^N)で
// コストの総和cost_sum_lbound以上かつcost_sum_ubound以下における
...
template <typename INT1 , typename INT2> inline pair<INT1,INT2> NegativeKnapsack( const vector<INT1>& value , const INT1& value_sum_ubound , const vector<INT2>& cost , const INT2& cost_sum_lbound , const INT2& cost_sum_ubound );
