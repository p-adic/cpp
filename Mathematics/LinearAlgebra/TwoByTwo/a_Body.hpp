...
template <typename T> inline TwoByTwoMatrix<T>& TwoByTwoMatrix<T>::operator*=( const T& scalar ) noexcept { m_M00 *= scalar; m_M01 *= scalar; m_M10 *= scalar; m_M11 *= scalar; return *this; }
...
template <typename T> inline TwoByTwoMatrix<T> TwoByTwoMatrix<T>::Square() const noexcept { return TwoByTwoMatrix<T>( m_M00 * m_M00 + m_M01 * m_M10 , ( m_M00 + m_M11 ) * m_M01 , m_M10 * ( m_M00 + m_M11 ) , m_M10 * m_M01 + m_M11 * m_M11 ); }

template <typename T> inline const T& TwoByTwoMatrix<T>::GetEntry( const uint& y , const uint& x ) const noexcept { return y == 0 ? x == 0 ? m_M00 : m_M01 : x == 0 ? m_M10 : m_M11; }
template <typename T> inline T& TwoByTwoMatrix<T>::RefEntry( const uint& y , const uint& x ) noexcept { return y == 0 ? x == 0 ? m_M00 : m_M01 : x == 0 ? m_M10 : m_M11; }
...
template <typename T> inline TwoByTwoMatrix<T> operator-( const TwoByTwoMatrix<T>& mat1 , const TwoByTwoMatrix<T>& mat2 ) noexcept { return move( TwoByTwoMatrix<T>( mat1 ) -= mat2 ); }
template <typename T> inline TwoByTwoMatrix<T> operator*( const T& scalar , const TwoByTwoMatrix<T>& mat ) noexcept { return move( TwoByTwoMatrix<T>( mat ) *= scalar ); }
template <typename T> inline TwoByTwoMatrix<T> operator*( const TwoByTwoMatrix<T>& mat , const T& scalar ) noexcept { return move( TwoByTwoMatrix<T>( mat ) *= scalar ); }
...
template <typename T> inline TwoByTwoMatrix<T> Square( const TwoByTwoMatrix<T>& mat ) noexcept { return mat.Square(); }
