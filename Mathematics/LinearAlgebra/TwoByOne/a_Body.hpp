...
template <typename T> inline TwoByOneMatrix<T>& TwoByOneMatrix<T>::operator+=( const TwoByOneMatrix<T>& mat ) noexcept { m_M0 += mat.m_M0; m_M1 += mat.m_M1; return *this; }
template <typename T> inline TwoByOneMatrix<T>& TwoByOneMatrix<T>::operator-=( const TwoByOneMatrix<T>& mat ) noexcept { m_M0 -= mat.m_M0; m_M1 -= mat.m_M1; return *this; }
...
template <typename T> inline TwoByOneMatrix<T>& TwoByOneMatrix<T>::operator*=( const T& scalar ) noexcept { m_M0 *= scalar; m_M1 *= scalar; return *this; }
template <typename T> inline TwoByOneMatrix<T>& TwoByOneMatrix<T>::operator/=( const T& scalar ) { m_M0 /= scalar; m_M1 /= scalar; return *this; }
template <typename T> inline TwoByOneMatrix<T>& TwoByOneMatrix<T>::operator%=( const T& scalar ) { m_M0 %= scalar; m_M1 %= scalar; return *this; }

inline const T& TwoByOneMatrix<T>::GetEntry( const uint& y ) const noexcept { return y == 0 ? m_M0 : m_M1; }
inline T& TwoByOneMatrix<T>::RefEntry( const uint& y ) const noexcept { return y == 0 ? m_M0 : m_M1; }
