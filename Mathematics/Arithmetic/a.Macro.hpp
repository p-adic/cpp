...
#define DECLARATION_OF_ARITHMETIC_FOR_MOD( FUNC , FUNC_CONSTEXPR )			\
  ...
  inline constexpr Mod<M> FUNC_CONSTEXPR ## _constexpr( const Mod<M>& n ) noexcept \
...
#define DEFINITION_OF_ARITHMETIC_FOR_MOD( FUNC , FORMULA , FUNC_CONSTEXPR , FORMULA_CONSTEXPR ) \
  ...
  template <INT_TYPE_FOR_MOD M> inline constexpr Mod<M> Mod<M>::FUNC_CONSTEXPR ## _constexpr( const Mod<M>& n ) noexcept { return Derepresent( FORMULA_CONSTEXPR ); } \
