...
template <typename LD , typename INT> auto PrimeFactorisation( const LD& ld , INT n ) -> enable_if_t<!IsPE<LD>,pair<vector<int>,vector<int>>>;
...
// O(nの素因数の指数の総和)、特にO(log n)
// LDはLeastDivisorなどで、n以下の整数の最小の素因数がldで管理されている時のみサポート。
template <typename LD , typename INT> auto PrimePowerFactorisation( const LD& ld , INT n ) -> enable_if_t<!IsPE<LD>,tuple<vector<int>,vector<int>,vector<INT>>>;
