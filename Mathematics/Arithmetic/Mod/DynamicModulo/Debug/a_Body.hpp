...
template <int NUM> template <typename T> inline DynamicMods<NUM>::DynamicMods( T n ) noexcept : m_non_negative( n >= 0 ) , m_n( uint( move( ( m_non_negative ? n : n *= -1 ) %= Constants::g_M ) ) ) , m_d( 1 ) { static_assert( is_constructible_v<uint,decay_t<T> > ); }
...
template <int NUM> inline DynamicMods<NUM>& DynamicMods<NUM>::operator+=( const DynamicMods<NUM>& n ) noexcept { ull m_n0 = ull( m_n ) * n.m_d , m_n1 = n.m_n * ull( m_d ) , m_n_copy = m_non_negative == n.m_non_negative ? move( m_n0 += m_n1 ) : m_n0 == m_n1 ? 0 : m_n0 > m_n1 ? move( m_n0 -= m_n1 ) : ( m_non_negative = !m_non_negative , move( m_n1 -= m_n0 ) ); ull m_d_copy = ull( m_d ) * n.m_d; ull gcd = GCD( m_n_copy , m_d_copy ); m_n = uint( move( ( m_n_copy /= gcd ) %= Constants::g_M ) ); m_d = uint( move( ( m_d_copy /= gcd ) %= Constants::g_M ) ); return *this; }
template <int NUM> inline DynamicMods<NUM>& DynamicMods<NUM>::operator-=( const DynamicMods<NUM>& n ) noexcept { ull m_n0 = ull( m_n ) * n.m_d , m_n1 = n.m_n * ull( m_d ) , m_n_copy = m_non_negative != n.m_non_negative ? move( m_n0 += m_n1 ) : m_n0 == m_n1 ? 0 : m_n0 > m_n1 ? move( m_n0 -= m_n1 ) : ( m_non_negative = !m_non_negative , move( m_n1 -= m_n0 ) ); ull m_d_copy = ull( m_d ) * n.m_d; ull gcd = GCD( m_n_copy , m_d_copy ); m_n = uint( move( ( m_n_copy /= gcd ) %= Constants::g_M ) ); m_d = uint( move( ( m_d_copy /= gcd ) %= Constants::g_M ) ); return *this; }
template <int NUM> inline DynamicMods<NUM>& DynamicMods<NUM>::operator*=( const DynamicMods<NUM>& n ) noexcept { ull m_n_copy = ull( m_n ) * n.m_n , m_d_copy = ull( m_d ) * n.m_d; ull gcd = GCD( m_n_copy , m_d_copy ); m_n = uint( move( ( m_n_copy /= gcd ) %= Constants::g_M ) ); m_d = uint( move( ( m_d_copy /= gcd ) %= Constants::g_M ) ); m_non_negative = m_n == 0 || m_non_negative == n.m_non_negative; return *this; }
...
template <int NUM> inline bool DynamicMods<NUM>::operator==( const DynamicMods<NUM>& n ) const noexcept { return m_non_negative == n.m_non_negative && m_n == n.m_n && m_d == n.m_d; }
...
template <int NUM> inline DynamicMods<NUM>& DynamicMods<NUM>::Invert() { assert( m_n != 0 ); std::swap( m_n , m_d ); return *this; }
...
template <int NUM> template <typename INT> inline DynamicMods<NUM>& DynamicMods<NUM>::Power( INT exponent ) { bool neg = exponent < 0; assert( !( neg && m_n == 0 ) ); return neg ? PositivePower( move( exponent *= ConstantsForDynamicMods<NUM>::g_order_minus_1_neg ) ) : NonNegativePower( move( exponent ) ); }
...
template <int NUM> inline uint DynamicMods<NUM>::Represent() const noexcept { DynamicMods<NUM> d_inv = DynamicMods<NUM>::Derepresent( m_d ).NonNegativePower( Constants::g_order_minus_1 ); return ull( m_n ) * d_inv.m_n % Constants::g_M; }
...
template <int NUM> inline string to_string( const DynamicMods<NUM>& n ) noexcept { const uint& M = DynamicMods<NUM>::GetModulo(); return ( n.GetSign() ? "" : "-" ) + to_string( n.GetNumerator() ) + "/" + to_string( n.GetDenominator() ) + " + " + to_string( M ) + "Z ( = " + to_string( n.Represent() ) + " + " + to_string( M ) + "Z )"; }
