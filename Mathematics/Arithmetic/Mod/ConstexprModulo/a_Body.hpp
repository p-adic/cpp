...
template <INT_TYPE_FOR_MOD M> template <typename T> inline constexpr Mod<M>& Mod<M>::Power( T&& exponent ) { bool neg = exponent < 0; assert( !( neg && m_n == 0 ) ); return neg ? PositivePower( forward<T>( exponent *= ConstantsForMod<M>::g_M_minus_2_neg ) ) : NonNegativePower( forward<T>( exponent ) ); }
...
template <INT_TYPE_FOR_MOD M> inline Mod<M> Mod<M>::Combination( const INT_TYPE_FOR_MOD& n , const INT_TYPE_FOR_MOD& i ) noexcept { return Montgomery<M>::Combination( n , i ).Reduce(); }
