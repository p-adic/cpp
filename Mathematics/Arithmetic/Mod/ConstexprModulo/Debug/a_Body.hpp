...
template <INT_TYPE_FOR_MOD M , class Traits> inline basic_ostream<char,Traits>& operator<<( basic_ostream<char,Traits>& os , const Mod<M>& n ) { return os << ( n.GetSign() ? "" : "-" ) << n.GetNumerator() << "/" << n.GetDenominator() << " ≡ " << n.Represent(); }
