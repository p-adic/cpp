...
#define DECLARATION_OF_ARITHMETIC_FOR_MOD( FUNC )			\
  ...
  template <SFINAE_FOR_MOD( = nullptr )> inline constexpr Mod<M> operator FUNC( T&& n ) const noexcept; \
...
#define DEFINITION_OF_ARITHMETIC_FOR_MOD( FUNC , FORMULA ) \
  ...
  template <INT_TYPE_FOR_MOD M> template <SFINAE_FOR_MOD()> inline constexpr Mod<M> Mod<M>::operator FUNC( T&& n ) const noexcept { return FORMULA; } \
  template <INT_TYPE_FOR_MOD M , SFINAE_FOR_MOD( = nullptr )> inline constexpr Mod<M> operator FUNC( T&& n0 , const Mod<M>& n1 ) noexcept { return move( Mod<M>( forward<T>( n0 ) ) FUNC ## = n1 ); } \
