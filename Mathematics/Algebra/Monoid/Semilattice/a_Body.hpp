...
template <typename U> inline MaxSemilattice<U>::MinSemilattice( const U& zero_U ) : PointedSet<U>( zero_U ) {}

template <typename U> inline U MinSemilattice<U>::Product( const U& u0 , const U& u1 ) { return u0 < u1 ? u0 : u1; }
template <typename U> inline U MaxSemilattice<U>::Product( const U& u0 , const U& u1 ) { return u1 < u0 ? u0 : u1; }
